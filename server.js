...
app.post('/regisDB', async (req, res) => {
  let sql = "CREATE TABLE IF NOT EXISTS userInfo (id INT AUTO_INCREMENT PRIMARY KEY, User_Date TIMESTAMP, User_Name VARCHAR(300), User_Email VARCHAR(300), User_Username VARCHAR(300), password VARCHAR(300))";
  let result = await queryDB(sql);
  sql = `Select User_Username from userInfo`;
  ...
  if (req.body.password == req.body.confirm_password && checkRegister == false) {
    ...
    sql = `INSERT INTO userInfo (User_Date, User_Name, User_Email, User_Username, password) VALUES ("${now_date}","${req.body.name}","${req.body.email}","${req.body.username}", "${req.body.password}")`;
    result = await queryDB(sql);
    check = true;
    checkRegister = true;
...
app.post('/CheckLogin', async (req, res) => {
  let sql = "CREATE TABLE IF NOT EXISTS userInfo (id INT AUTO_INCREMENT PRIMARY KEY, User_Date TIMESTAMP, User_Name VARCHAR(300), User_Email VARCHAR(300), User_Username VARCHAR(300), password VARCHAR(300))";
  let result = await queryDB(sql);
  sql = `Select User_Username, password from userInfo`;
  result = await queryDB(sql);
  result = Object.assign({}, result);
  var keys = Object.keys(result);
  var check = false;
  ...
  for (var counter = 0; counter < keys.length; counter++) {
    if (req.body.username == result[keys[counter]].User_Username &&
      ...
      console.log(res.cookie("username", result[keys[counter]].User_Username));
...
app.post('/UpdateDB', async (req, res) => {
  ...
  for (var information_num = 0; information_num < keys.length; information_num++) {
    if (req.body.username == result[keys[information_num]].User_Username &&
      ...
      let sql = `UPDATE userInfo SET password = '${req.body.password}' WHERE User_Username = '${req.body.username}'`;
      let result = await queryDB(sql);
      check = true;
      return res.redirect('html/login.html');
    }
